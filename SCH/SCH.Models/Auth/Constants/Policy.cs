namespace SCH.Models.Auth.Constants
{
    public static class Policy
    {
        public const string ViewStudents = "ViewStudents";
        public const string AddStudents = "AddStudents";
        public const string EditStudents = "EditStudents";
        public const string EditOwnStudent = "EditOwnStudent";
        public const string DeleteStudents = "DeleteStudents";

        public const string ViewTeachers = "ViewTeachers";
        public const string EditTeachers = "EditTeachers";
        public const string EditOwnTeacher = "EditOwnTeacher";
        public const string DeleteTeachers = "DeleteTeachers";

        public const string ViewCourses = "ViewCourses";
        public const string AddCourses = "AddCourses";
        public const string EditCourses = "EditCourses";
        public const string DeleteCourses = "DeleteCourses";
    }
}
